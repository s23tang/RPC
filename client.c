...
int main() {
  ...
  argTypes3[0] = (1 << ARG_OUTPUT) | (ARG_INT << 16);
  argTypes3[1] = (1 << ARG_INPUT) | (ARG_INT << 16);
  argTypes3[2] = (1 << ARG_INPUT) | (ARG_INT << 16);
  argTypes3[3] = 0;
    
  args3 = (void **)malloc(count3 * sizeof(void *));
  args3[0] = (void *)&return3;
  args3[1] = (void *)&a3;
  args3[2] = (void *)&b3;
  ...
  int s3 = rpcCall("f0", argTypes3, args3);
  /* test the return f0 */
  printf("\nEXPECTED return of f0 is: %d\n", a3 + b3);
  if (s3 >= 0) { 
    printf("ACTUAL return of f0 is: %d\n", *((int *)(args3[0])));
